<x-layouts.base :title="$title ?? null">
    <div class="flex min-h-screen flex-col">
        <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-lg dark:border-slate-700 dark:bg-slate-900/95">
            <div class="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ route('dashboard') }}" class="rounded-lg bg-cyra-600 px-3 py-2 text-sm font-medium text-white hover:bg-cyra-700 sm:px-4">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="text-sm font-medium text-slate-600 hover:text-cyra-600 dark:text-slate-300">Log in</a>
                            <a href="{{ route('register') }}" class="rounded-lg bg-cyra-600 px-3 py-2 text-sm font-medium text-white hover:bg-cyra-700 sm:px-4">Get Started</a>
                        @endauth
                    @endif
                </div>
            </div>
        ...
        <footer class="border-t border-slate-200 px-4 py-6 text-center text-xs text-slate-500 sm:text-sm dark:border-slate-700">
            &copy; {{ date('Y') }} CyraConnect. Africa's Innovation Operating System.
        </footer>
    </div>
</x-layouts.base>
